...
| deployment.skaha.sessions.authorization.permissionsAPI.authAPIBaseURL | string | `""` | Base URL for the IAM/OIDC auth API used by the permissions client. Required when enabled is true (Helm validates). |
...
| deployment.skaha.sessions.limitRange.create | bool | `true` | When true together with `enabled`, the chart creates the LimitRange `<release>-session-limit-range` in the workload namespace from `spec` below. When false, skip creating it and assume one with that name already exists (for example provisioned out-of-band). |
| deployment.skaha.sessions.limitRange.enabled | bool | `false` | When true, set `SKAHA_SESSION_LIMIT_RANGE_ENABLED` on the Skaha deployment and allow this chart to emit the LimitRange Role/RoleBinding (`rbac.create`). Must also be true (with `create`) for the chart to render the LimitRange manifest. |
