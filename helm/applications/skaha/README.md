...
| deployment.skaha.sessions.limitRange.create | bool | `true` | When true, the chart creates the LimitRange `<release>-session-limit-range` in the workload namespace from `spec` below. When false, skip creating it and assume one with that name already exists (for example provisioned out-of-band). |
| deployment.skaha.sessions.limitRange.enabled | bool | `false` | When true, set `SKAHA_SESSION_LIMIT_RANGE_ENABLED` on the Skaha deployment so the service loads the namespace LimitRange for resource context. Independent of `create`. |
